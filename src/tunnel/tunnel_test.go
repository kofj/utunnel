...
func TestTunnelRC4(t *testing.T) {
    ...
    b := NewTunnel("127.1:9446", "127.1:9480", false, "rc4", "secret", 4)
...
func TestTunnelAES256CFB(t *testing.T) {
    ...
    b := NewTunnel("127.1:9448", "127.1:9481", false, "aes256cfb", "secret", 4)
    f := NewTunnel("127.1:9449", "127.1:9448", true, "aes256cfb", "secret", 4)
    go b.Start()
    go f.Start()
    // sleep to wait all servers start
    time.Sleep(100 * time.Millisecond)
    conn, err := net.Dial("tcp", "127.1:9449")
    if err != nil {
        t.Error(err)
    }
    defer conn.Close()
...
func backendServer(port int, data []byte, done chan bool, t *testing.T) {
    ln, err := net.Listen("tcp", fmt.Sprintf("127.1:%d", port))
    if err != nil {
        t.Error(err)
    }
    defer ln.Close()
